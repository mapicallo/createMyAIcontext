/**
 * Create my AI Context — floating panel (AI4Context family pattern).
 */
const PANEL_PAGE = 'panel.html';
const SESSION_PANEL_KEY = 'cmac_panel_window_id';
...
async function openPanel(): Promise<void> {
  ...
  const attempts: chrome.windows.CreateData[] = [
    { url: panelUrl, type: 'popup', width: 460, height: 760, focused: true },
    { url: panelUrl, type: 'normal', width: 480, height: 780, focused: true },
  ];
  ...
  for (const createData of attempts) {
    try {
      ...
      console.warn('[Create my AI Context] window create failed', createData.type, e);
    }
  }
  ...
  try {
    ...
    console.error('[Create my AI Context] could not open panel', e);
  }
}
